...
def arch_top(config, **kwargs):
    if config['arch_top'] in models.network_names:
        net = models.network_names[config['arch_top']](**config['arch_kwargs'], **kwargs)
    else:
        raise ValueError(f'Invalid Arch: {config["arch_top"]}')

    return net

def arch_bottom(config, **kwargs):
    if config['arch_bottom'] in models.network_names:
        net = models.network_names[config['arch_bottom']](**config['arch_kwargs'], **kwargs)
    else:
        raise ValueError(f'Invalid Arch: {config["arch_bottom"]}')

    return net


def optimizer_top(config, params):
    o_type = config['optim']
...
def optimizer_bottom(config, params):
    o_type = config['optim']
    kwargs = config['optim_kwargs']
